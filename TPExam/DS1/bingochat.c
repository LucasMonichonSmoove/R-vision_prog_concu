...
void* player_thread(void* arg) {
    ...
    while (1) {
        // attendre qu'un numéro soit tiré
        pthread_barrier_wait(barrier);

        // si la partie est finie, sortir proprement
        if (*winner != -1) {
            pthread_barrier_wait(barrier);
            break;
        }

        // vérifier une seule fois la grille
        ...
        // signaler au main qu'on a terminé ce tour
        pthread_barrier_wait(barrier);
    }
...
int main(int argc, char* argv[])
{
    ...
    while (winner == -1 && nb_draw != MAX) {
        /* Zone à Modifier */
        // ici on ne met rien avant le tirage
        /* Fin Zone à Modifier */
        ...
        /* Zone à Modifier */
        // réveiller les joueurs pour ce numéro
        pthread_barrier_wait(&barrier);

        // attendre que tous aient fini la vérification
        pthread_barrier_wait(&barrier);
        /* Fin Zone à Modifier */
    }

    /* Zone à Modifier */
    // débloquer les threads s'ils sont encore en attente
    pthread_barrier_wait(&barrier);
    pthread_barrier_wait(&barrier);
    ...
    if (winner != -1 && check_winner(draw, MAX, grid[winner], GRID_SIZE)) {
